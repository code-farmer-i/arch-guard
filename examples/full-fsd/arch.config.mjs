...
export default {
  ...
  presets: [
    ...
    callSites([
      { name: '本地存储', apis: ['localStorage'], in: ['src/shared/lib/storage.ts'] },
      { name: '埋点上报', apis: ['gtag'], in: ['src/shared/lib/analytics/**'] },
      { name: '全局单例', apis: ['QueryClient'], in: ['src/shared/api/queryClient.ts'] },
    ]),
  ],
