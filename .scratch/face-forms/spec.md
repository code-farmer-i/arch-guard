...
## 收尾（第五轮：§7.2 的两块真缺口 —— 字面量唯一出处）

DESIGN §7.2 行 2 / 行 3 里"尚未实现"的两块（不是"写死形态"，是**规则本身不存在**）：

| 规则                     | 落点（面字段）            | 形态（面字段）                                                                 | 判据                                   |
| ------------------------ | ------------------------- | ------------------------------------------------------------------------------ | -------------------------------------- |
| **D22** 缓存键唯一出处   | `data-layer.queryKeyFrom` | `queryKeyProps`（默认 `['queryKey']`）                                         | facts 的 `strings[].prop`              |
| **D23** 路由路径唯一出处 | `router.pathSource`       | `pathProps`（`['path','to']`）+ `navigateCalls`（`navigate` / `router.push`…） | `strings[].prop` + `calls[].stringArg` |

三个关键决定：

1. **落点是项目事实**（同 `designSystem({ styleDir })`）：由 kit 选项传入，没声明 → `requires` 明列停用；
   落点写错（文件不存在）→ **只报一条"落点不存在"**，不逐条刷项目（逐条报会把真正的问题埋掉）。
2. **`StringFact.prop` 改成透传**：`queryKey: ['crews', id]` 的字面量直接父节点是数组，名字在爷爷那层；
   数组 / 对象 / 括号 / 断言 / 展开 / 三元不改变它，函数体与调用实参断开。这也是那个字段的**第一个消费者**
   （此前 `context` / `prop` 两个字段零消费者 —— 属于"声明了没人读"，正好借这两条规则补上）。
   ⇒ `FACTS_CACHE_SPEC` 1 → **2**（语义变了，旧缓存必须作废，否则静默不判）。
3. **域预设也要列**：`designSystem()` 的 enable 必须等于 D 域全部实现规则（`engine-unit` 守卫盯着），
   于是 D22 / D23 两处都列（域预设 + 方案面预设），并集不冲突；真正的开关是能力而不是 enable。

边界（写进 DESIGN §7.2(4)）：位置参数形态的缓存键（`useSWR('/api/crews')`）**不在 v1** ——
要支持得再加一种形态字段，在那之前 `queryKeyFrom` 只覆盖具名属性那一半。
