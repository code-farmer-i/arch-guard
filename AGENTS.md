...
| `examples/**`     | 宿主示例（都是**可跑的活样板**）：`minimal`（最短可用，69/98 规则在跑）· `full`（范式 + 5 域 + 8 面 + 结构声明全配，**93/98 在跑、0 finding** —— 差的是覆盖率那 5 条）。`pnpm guard:sample` / `guard:full` 跑它们（已挂在 `pnpm check` 里） |
