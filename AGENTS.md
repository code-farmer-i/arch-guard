...
| `examples/**`     | 宿主示例（都是**可跑的活样板**）：`minimal`（最短可用，69/98）· `full`（`canonical()` + 5 域 + 8 面 + 结构声明全配，**93/98、0 finding**）· `full-fsd`（`fsd()` 版，**82/98、0 finding**）。`pnpm guard:sample` / `guard:full` / `guard:full-fsd` 跑它们（已挂在 `pnpm check` 里） |
