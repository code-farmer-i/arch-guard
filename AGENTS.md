...
1. **别信"✅ Published"** —— 发布工具的消息不等于产物上线。**必须验权威源**（10 秒）：
   ...
   **验"上线的是不是这一版"**：装到临时目录后，除了 `--version`，再 `grep` 几个**这一版新增的内部标识**
   （如 `es/` 里的常量名 / 信号 id）—— 比只看版本号更能证明产物就是这一版
   （0.7.0 实测：`TEST_LAYER_MIN` / `isTestPath` / `FACET_FIELDS` / `group-chain-depth` 全在 ✓）。
