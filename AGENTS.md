...
| `examples/**`     | 宿主示例（都是**可跑的活样板**）：`minimal`（最短可用，71/104）· `full`（`canonical()` + 4 个域 + 10 个生效适配器 + 结构声明全配，**99/104、0 finding**）· `full-fsd`（`fsd()` 版、六层含 widgets，**88/104、0 finding**）。`pnpm guard:sample` / `guard:full` / `guard:full-fsd` 跑它们（已挂在 `pnpm check` 里） |
