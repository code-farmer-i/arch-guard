...
| ~~假异步、`Math.random`、硬编码地址、假数据字面量~~（**0.4.0 收回本体**：H07 warn / H08 error / H09 warn，声明的形态才判） | H07–H09           | —                                                                                                                                                                               |
...
> 本表是**完整设计**（含未实现项）。**已实现并带夹具的 94 条**（以 `arch-guard --stats` / `coreRules` 为准）：
...
| ID  | 红线                                                                                                                                                                                                                                                                                                                                                                                                                     | 判据                        | 等级  | 级别  |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------- | ----- | ----- |
| C01 | **裸文案**：JSX 文本节点（`context: 'jsx'`）、面向用户的属性（`title` / `placeholder` / `alt` / `aria-label` / `label`），以及**声明了 `messageApis` 时的组件库调用字符串实参**（`message.success('保存')`）—— **0.4.0 收回本体**（原先委派 `eslint-plugin-i18next`；"这句走没走 `t()`"从 `facts.calls[].stringArg` 就知道）。**声明了 i18n 才判**；命中给 warn。已知边界：对象实参形态（`{ message: '保存' }`）还看不见 | 事实（`strings` / `calls`） | L2    | warn  |
