...
> 本表是**完整设计**（含未实现项）。**已实现并带夹具的 95 条**（以 `arch-guard --stats` / `coreRules` 为准）：
> `S00–S06`、`S08`、`S09`、`S11–S32`、`S34–S44`、`D01–D10`、`D10b`、`D11–D14`、`D16–D18`、`D21–D24`、`C01–C07`、`P01–P08`、`P11`、`P12`、`H06–H09`、`H12`、`H13`、`M02–M09`。
> 不在本体跑的：`S10`（相对越级，0.4.0 由 S43 取代）与 `H01–H05`、`H10`、`D15`、`D19`、`D20` —— 前一类按 §4.9 **委派**给 eslint / dependency-cruiser / knip；
> 后几类**已设计未实现**（H10 的手搓时间格式化现由 P06 能力指纹代管）。**P03 / P08 原委派给 knip · depcheck，0.4.0 已收回本体**（配置成本高而事实现成）。
...
| D 域 | **22 条已落地**（`coreRules` 的 design 域全部；0.4.0 从 stylelint 收回 D01 · D02 · D09 · D12–D14 · D18）                                                                                                                | **D15 · D19 · D20 未实现**：内联样式纪律、跨文件重复数值提示、请求策略数字登记这三类目前没有红线 |
