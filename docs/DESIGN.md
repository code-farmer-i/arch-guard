...
> 本表是**完整设计**（含未实现项）。**已实现并带夹具的 107 条**（以 `arch-guard --stats` / `coreRules` 为准）：
> `S00–S06`、`S08`、`S09`、`S11–S32`、`S34–S46`、`D01–D10`、`D10b`、`D11–D20`、`D21–D30`、`C01–C07`、`P01–P08`、`P11`、`P12`、`H06–H09`、`H12`、`H13`、`M02–M10`。
...
> **P03 / P08 原委派给 knip · depcheck，0.4.0 已收回本体**（配置成本高而事实现成）；
...
| S23 | **公开面**（`@x` 文件不算组的公开入口，但**被指名的调用方**可以直接引它 —— 与 S22 同一条例外，R-105）：组必须有入口（角色描述符 `entry: true`），且组外不许直接引用组内非入口文件（`structure.publicApi` 声明了才判）                                                                                                                                                   | 依赖图                | L3    | error                                                                                                        |
...
| S46 | **埋点上报不许写在渲染体里**：`analytics({ apis })` 里**非 hook** 的上报调用出现在组件渲染体（`functions[].isComponent` 的最内层函数）即报 —— 渲染体每次重渲染都执行，StrictMode 还双调用；D24 只看事件名是不是字面量，看不见调用位置                                                                                                                                   | `calls` + `functions` | L2    | error                                                                                                        |
...
| S38 | **调用只在声明的落点**（组可加 `args: [...]` 收窄到**第一个字符串字面量实参**：`searchParams.get` 这类泛 API 只判 `get('tenantId')`，不判 `get('page')` —— R-110）：按 `callSites([{ name, apis, in }])` 声明的组判 —— **副作用**（埋点/上报 SDK、本地存储）与**配置对象**（`new QueryClient()` / `createTheme()`）出现在落点外就报（对象方法按前缀匹配；测试文件豁免） | 调用 + glob 落点      | L2    | error（声明才判）                                                                                            |
...
| D22  | **缓存键只有一个出处**：`{ queryKey: ['crews', id] }` 这类键字面量只许出现在 `dataLayer({ queryKeyFrom })` 声明的落点里 —— **落点可以是数组、每项可以是 glob**（R-97：键跟着域/切片走时一行覆盖所有域；含数组内的字面量，靠 facts 的 `strings[].prop` 透传拿属性名）                                                       | AST 字面量 + 路径                                        | L2                                         | error（声明落点才判）                                                                        |
| D23  | **路由路径只有一个出处**：`path` / `to` 属性与 `navigate()` 这类跳转调用的**绝对路径字面量**只许出现在 `router({ pathSource })` 声明的文件里                                                                                                                                                                               | AST 字面量 + 路径                                        | L2                                         | error（声明落点才判）                                                                        |
| D27  | **对比度组合必须声明过**：组件样式里同时写 `color` 与 `background`（都是令牌引用）时，这一对必须在 `contrastPairs` 里 —— D07 只算**声明过的那几对**，于是"语义令牌当反色用"（白字橙底 3.1:1）永远不会被算到                                                                                                                | CSS declarations + 声明                                  | L2                                         | error                                                                                        |
| D28  | **声明的样式落点必须有入口**：`styleDir` / `tokenDir` / `vendorDir` 下至少有一份 CSS 能被**应用侧**（非 CSS）引用到 —— 沿 `@import` 链上溯；真实踩过：四份全局 CSS 都在，但谁也没引，主题静默不生效而门禁全绿                                                                                                              | CSS 图（importers） + 声明                               | L2                                         | error                                                                                        |
| D29  | **权限点只有一个出处**：判权限点的调用（`permissions({ apis, source })`）里出现**字面量权限点**（`can('crew:edit')`）即报 —— 与 R-46（S38 管"判断写在哪儿"）分工：这条管"点叫什么、在哪儿定义"                                                                                                                             | `calls[].stringArg` + 声明                               | L2                                         | error                                                                                        |
| D30  | **失败处理的策略只有一个出处**：属性名命中策略名单（声明或 kit 给）、值是**函数**（`retry: (n) => …`）或**字符串枚举**（`backoff: 'exponential'`）、且**作为调用实参**时，只许出现在声明的策略落点 —— 与 D20 分工：数字型归 `numberHomes`，函数体里的口径归这条（同一处不会两条都报）                                      | `functions[].ownedProp/inCall` + `strings[].prop/inCall` | L2                                         | error                                                                                        |
| D25  | **后端端点只有一个出处**：**打后端的调用**（`endpoints({ apis?, from?, source })` 声明，如 `fetch`）实参里出现路径字面量（`/crews`）即报；`source` 收**一个或一组文件（glob，R-116）**，某个落点零命中只报"落点不存在"不刷屏 —— 端点以前是"事实模型里的隐形人"（多写在模板串里），事实模型为此扩了 `calls[].templateParts` | `calls[].templateParts` + 声明                           | L2                                         | error                                                                                        |
| D26  | **缓存键形状一致**：同一个键工厂（`export const crewKeys = { … }`）里各键数组的**首元素**必须相同 —— 否则 `invalidateQueries({ queryKey: ['crews'] })` 前缀匹配不上，缓存不失效、数据不刷新，而 D22 只管"字面量在不在家"（事实扩了 `strings[].arrayPath`）                                                                 | `strings[].arrayPath` + 声明                             | L2                                         | error                                                                                        |
...
| ID  | 红线                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | 判据                        | 等级  | 级别  |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------- | ----- | ----- |
| C01 | **裸文案**：三种形态 —— JSX 文本节点（`context: 'jsx'`）、面向用户的属性（`title` / `placeholder` / `alt` / `aria-label` / `label`）、以及**组件库调用**（直接实参 `message.success('保存')` + 对象实参 `notification.open({ message: '保存' })`）—— 名单来自**组件库适配器**（`uiKit.messageApis` / `messageProps`，如 `antdKit()` 自带 antd 那份），项目自己的封装由 `copy({ messageApis })` **覆盖**（传 `[]` = 关掉这一半）—— **0.4.0 收回本体**（原先委派 `eslint-plugin-i18next`）。**声明了 i18n 才判**；命中给 warn | 事实（`strings` / `calls`） | L2    | warn  |
| C02 | 每个 `t('k')` 的 key 必须中英都存在（**复数感知**：`t('ns.count')` 认 `ns.count_one` / `ns.count_other`，R-104）                                                                                                                                                                                                                                                                                                                                                                                                            | locales 索引                | L3    | error |
| C03 | 两份语言目录同构（文件集合一致 + 逐文件键一致）；**复数按基键对账** —— 英文有 `_one`/`_other`、中文只有 `_other` 本来就该不一样（R-104）                                                                                                                                                                                                                                                                                                                                                                                    | 目录配对                    | L1+L2 | error |
| C04 | 一文件一顶层命名空间，键前缀 = 文件名                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | AST + 路径                  | L1+L2 | error |
| C05 | locales 文件必须被 `i18n/index` 聚合                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | 图                          | L3    | error |
| C06 | 无死键（未被 `t()` 引用）；复数变体由**基键**的使用点亮（`t('ns.count')` 用到 `count_one` / `count_other`，R-104）                                                                                                                                                                                                                                                                                                                                                                                                          | 引用完整性                  | L3    | warn  |
...
| H10 | 手搓时间格式化（`toLocale*String` / `Intl.DateTimeFormat`）→ 用 dayjs。**没有独立的 H10 规则**：判据在数据表 `data/wheel-fingerprints.ts` 的 `capability: 'datetime'` 里，由 **P06** 消费 —— 项目声明 `deps({ capabilities: { datetime: 'dayjs' } })` 才判（不声明不判：不用日期库的项目用原生 `Intl` 是正当的） | AST          | L2   | warn（经 P06 落地；`toISOString()` 这类机器序列化不判）                                                      |
...
- **JSX 可访问性形态**（`<div onClick>` 可点不可聚焦 / 缺 `alt` / 缺 `aria-label` / 正数 `tabIndex` / `role` 乱用）
  —— **判不做**（REQUIREMENTS 第八节 N-09）：交给宿主自装 `eslint-plugin-jsx-a11y`（单文件语法形态、零项目专有数据，与 H01–H05 同类）。
  本体只碰可判定的那一半：**D07 对比度基线**（令牌对）· **C01**（`alt` / `aria-label` 的**文案**必须走 `t()`）
...
`uiKit.icons` → P05 · `uiKit.packages` → P11 · `uiKit.detachedApis` → H06 · `uiKit.messageApis` / `uiKit.messageProps` → C01）。两者缺一不可 ——
...
| `messageApis`     | 该库的**文案位调用**（`message.success` / `notification.open`…）    | C01（组件库调用那一半）            |
| `messageProps`    | 该库里**哪些对象属性算文案**（`message` / `description` / `title`） | C01（对象实参那一半）              |
...
export default () => ({
  ...
  vendorSelectors: ['\\.ant-'],
  vendorVars: ['^--ant-'],
  // C01 的文案位名单：**库的事实**，宿主不必抄（项目自己的封装用 copy({ messageApis }) 覆盖）
  messageApis: ['message.success', 'notification.open', 'Modal.confirm'],
...
| --- | ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------- |
| 1   | UI 组件库           | §7.1 适配表 ✅                                                                                                                                                                                                                        | —                                                                                                                                                      | 已做              |
| 2   | **数据层**          | **适配器已建**（`dataLayer()` + P12 + **D22 缓存键唯一出处** + **S36 取数只在声明的落点**）：`{ queryKeyFrom }` 声明键的落点（可多落点/glob）、`{ fetchIn }` 声明取数落点 —— 页面里直接 `useQuery`、域里直接 `fetch('/api/x')` 都会报 | 换 SWR / Jotai：`queryKeyProps` / `fetchApis` 一改就跟着换（位置参数形态如 `useSWR('key')` 仍不在 v1，见 §7.2(4)）；没声明落点 → D22 / S36 明列停用    | T1 已做           |
...
| `data-layer` | `queryKeyFrom` / `queryKeyProps`（键落点与属性名）             | D22                         | 落点空 → 停用；`['queryKey']`；**落点收字符串或数组，每项可 glob** |
