...
> 需求层面的「明确不做」登记在 [`REQUIREMENTS.md`](../REQUIREMENTS.md) 第七节 —— 这里只列**本体的**非目标，别在两处各写一份。
...
> 本表是**完整设计**（含未实现项）。**已实现并带夹具的 90 条**（以 `arch-guard --stats` / `coreRules` 为准）：
...
| ID  | 红线                                                                                                                                                                          | 判据                     | 等级  | 级别                                                                               |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------ | ----- | ---------------------------------------------------------------------------------- |
| C01 | **JSX 裸文本禁止**：文本节点只许表达式（`{t(...)}`）；当源语言含 CJK 时，非 locales 文件禁中文字面量                                                                          | AST JSXText + 字符串     | L2    | error **0.4.0 收回本体**（warn；只判面向用户的属性，JSX 文本待事实模型收 JSXText） |
...
| H07 | 假异步与随机：`setTimeout` 模拟请求、`Math.random()`                                                                                                                                | AST          | L2   | error **0.4.0 收回本体**（warn，两证据同时成立才提示）                                                    |
...
| H09 | 静默假数据：`mock`/`fake`/`dummy`/`lorem` 字面量                                                                                                                                    | AST          | L2   | warn **0.4.0 收回本体**（warn，只按 mock/fake/dummy 命名判）                                              |
...
| R6   | 有规则级 `exceptions`，但没有「生成代码必须带 `@generated` 标记」的可证伪要求                                                                                                                                                                                                                                                       | 豁免可以被随意扩大                                 |
| D 域 | **14 条已落地**（D03–D08 / D10 / D10b / D11 / D16 / D17 / D21 / D22 / D23：色值唯一 / 令牌闭合 / 死令牌 / 明暗双份 / 对比度 / storage key / vendor 边界与反向封闭 / 无框架残留 / 样式落点 / CSS Module 双向契约 / 声明与事实对账 / 缓存键唯一出处 / 路由路径唯一出处）；**D01 · D02 · D09 · D12–D15 · D18 已委派后删除**（见 §4.9） | 委派出去的那半宿主要自己装并配好；没装等于失去覆盖 |
| C 域 | **6 条已落地**（C02–C07：键存在 / 多语言一致 / 一文件一命名空间 / 分片聚合 / 死键 / 声明与资源对账）；C01 裸文案委派给 `eslint-plugin-i18next`                                                                                                                                                                                      | 动态键（`t(`ns.${x}`)`）只按静态前缀放行，不做求值 |
