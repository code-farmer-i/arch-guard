...
> 本表是**完整设计**（含未实现项）。**已实现并带夹具的 106 条**（以 `arch-guard --stats` / `coreRules` 为准）：
> `S00–S06`、`S08`、`S09`、`S11–S32`、`S34–S46`、`D01–D10`、`D10b`、`D11–D20`、`D21–D30`、`C01–C07`、`P01–P08`、`P11`、`P12`、`H06–H09`、`H12`、`H13`、`M02–M09`。
...
| D25  | **后端端点只有一个出处**：**打后端的调用**（`endpoints({ apis, source })` 声明，如 `fetch`）实参里出现路径字面量（`/crews`）即报 —— 端点以前是"事实模型里的隐形人"（多写在模板串里），事实模型为此扩了 `calls[].templateParts`                                                        | `calls[].templateParts` + 声明                           | L2                                         | error                                                                                        |
