  ...
  packs/        框架包 = **源码形态**的落地（一个项目一个）
    core/           共享规则实现（框架无关）
      ...
      rules/          75 条规则的实现：structure / structure-graph / structure-declared /
                      ...
                      copy / deps / deps-adapters / hygiene-context / hygiene-retired / metrics
...
- 委派给生态的那半（DESIGN §4.9）：**宿主没装工具就等于没有那层覆盖**（需求 R-51 想把它变成可判定）。
...
| 文档                         | 回答什么                                     | 不回答什么               |
| ---------------------------- | -------------------------------------------- | ------------------------ |
| **本文件**                   | 本体架构：分层 / 数据流 / 模块职责 / 边界    | 规则判定细节、需求       |
| `PARADIGM.md`                | 宿主该长什么样（可搬的规范）                 | 本体怎么实现             |
| `docs/DESIGN.md`             | 规则清单、判定等级、配置协议、报告契约、选型 | 模块怎么分层（见本文件） |
| `REQUIREMENTS.md`            | 痛点 / 期望行为 / 状态 / 由谁来做            | 怎么实现                 |
| `CONTEXT.md`                 | 词汇表                                       | 设计与需求               |
| `docs/adr/**`                | 重要决策的**理由**（不可变记录）             | 现状                     |
| `.scratch/<slug>/spec.md`    | 单个需求的实现规格                           | 架构全貌                 |
| `AGENTS.md`                  | 给 agent 的操作指南（改代码前 / 提交前）     | 架构设计                 |
| `README.md` / `CHANGELOG.md` | 对外概览 / 每个版本发生了什么                | 架构与需求               |
