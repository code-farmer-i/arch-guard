...
| **想看「配全」长什么样**         | [`examples/full/`](../examples/full/) —— 一个范式 + 5 个域 + 8 个面 + 结构声明全配的最小应用：`pnpm guard:full` 跑出来是 **93/98 在跑、0 finding**（差的是 M02–M06 覆盖率那 5 条 —— 它们要一份比最近一次提交还新的产物，入库的必然过期；本仓自己的 `arch.config.mjs` 也正因此不声明 `coverage`）。最短可用的对照是 [`examples/minimal/`](../examples/minimal/)（69/98） |
