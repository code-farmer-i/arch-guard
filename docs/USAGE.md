...
**别一上来就照着 `examples/full` 抄 139 行**。这条路径是有顺序的：

| 步  | 做什么                                                                                                                                     | 你会得到             |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------ | -------------------- |
| ①   | 抄 [`examples/minimal`](../examples/minimal/) 的 **5 行**（范式 + 反退化 + 声明空 kit）                                                    | 69/98 规则在跑，绿   |
...
| **想看「配全」长什么样**         | 两份对称的活样板：[`examples/full/`](../examples/full/)（`canonical()` 三根拓扑，**93/98 在跑**）与 [`examples/full-fsd/`](../examples/full-fsd/)（`fsd()` 六层切片，**82/98**）。两者都是 0 finding；差的那几条：M02–M06 覆盖率五条要一份比 HEAD 新的产物（入库的必然过期，本仓自己也不声明 `coverage`），FSD 另有 S13 / S37 明列停用 + 9 条应用专属规则不适用。最短可用看 [`examples/minimal/`](../examples/minimal/)（69/98） |
