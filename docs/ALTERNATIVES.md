...
export default {
  ...
  overrides: {
    // 结构交给 steiger：只留兜底枚举 + 与目录规范无关的几条
    enable: [
      'S00',
      'S01',
      'S02',
...
1. **`enable` 现在是"覆盖"不是"并集"** —— 不写上面那份并集，`library()` 的白名单会把 D/C/M 域**静默关掉**（实测 10/53 条）。这是当前的阻塞点。
...
- 2026-09-23 成文：来自"成熟工具组合 → FSD 组合 → steiger 的边界 → 能否实现 FSD"这一串讨论。
  ...
  `entry: []` 的角色歧义均为**本地实测**。
- 待办：`enable` 改成并集（+`disable` 减法）—— 这是 §3.3 配方能否开箱可用的前提。
