...
`scripts/check-theme.mjs`（`lint:theme`）的能力**并入 design 域**，旧脚本退役。
...
> 本表是**完整设计**（含未实现项）。已实现并带夹具的规则：S00、S01、S02、S10–S14、S16、P01–P03、P06、P08、H01–H05。
...
| S16 | 体积：文件 ≤500（默认，可配）/ 单组件函数 ≤150 / 单文件导出值 ≤6 / 单文件组件数 ≤3 | AST 计数    | L2    | error/warn |
...
**只列还没做的。** 已落地的能力见 [`CHANGELOG.md`](../CHANGELOG.md)，进度见 [`README.md`](../README.md) 的 Roadmap。
...
| R8   | 别名可能有两处来源（tsconfig `paths` 与打包器 alias），没有 `--check-aliases`          | 图解析静默失败 → 孤儿误报/漏报                              |
| E2   | 适配器面清单（facet）仍硬编码在引擎里                                                  | 加一个新面要动引擎                                          |
| E3   | pack 只有职责描述，没有 `definePack()` 契约                                            | 「加 Vue 是加法」目前只是承诺，不是保证                     |
| E4   | 只有适配器带 `specVersion`，config / baseline 没有                                     | 契约变更时无法给 codemod 提示                               |
| E5   | 没有规则退役机制（`--stats` 命中统计）                                                 | 规则只增不减，长期零命中的规则没人敢删                      |
| E6   | `--format` 只有 `pretty` / `json`，缺 CI 注解（github）格式                            | CI 里要自己解析 JSON                                        |
| D 域 | 设计系统域 **0 条规则**（令牌图 / 明暗双份 / 对比度 / 魔法数字全未迁移）               | 样式类红线目前没有门禁                                      |
| C 域 | 文案域 **0 条规则**（键存在 / 双份同构 / 死键 / 裸文本）                               | 文案契约目前没有门禁                                        |
