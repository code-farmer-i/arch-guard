...
_注意：_
_- `:::demo-preview` 内部写的是 demo 文件名，不要带 `.vue` 后缀。_
_- 根据站点的 `demoPreview` 配置，它会将对应的代码渲染为真实的 Vue 组件并提供代码查看/折叠功能（`preview` 模式），或者仅展示代码面板（`codeOnly` 模式，通常配合右侧移动端模拟器使用）。_
...
```html
请参考
<PagodaDocLinkToView view="development/cli/commands">CLI 命令参考</PagodaDocLinkToView> 了解更多。
```
