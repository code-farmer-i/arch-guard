    ...
    nav: [
      {
        title: '组件',
        view: 'components',
        items: [{ title: 'Button 按钮', view: 'button' }],
      },
    ],
  },
})
...
3. **组件文档与 Demo (`src/button/README.md`)**:
   ...
   _注意：`:::demo-preview basic :::` 会自动寻找并渲染同目录 `demo/basic.vue` 文件。_
...
4. **创建 Demo 文件 (`src/button/demo/basic.vue`)**:
   ...
   ```
   _必须放在 `demo/` 目录下，文件名与 Markdown 中 `:::demo-preview` 引用的名称一致（不含 `.vue` 后缀）。_
