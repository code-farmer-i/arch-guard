...
- **顺序**：数组中的命令将严格按顺序同步执行。
- **执行目录**：钩子命令在项目根目录执行。
- **异常中断**：如果任何一个钩子命令返回非零退出码（失败），CLI 会**立即中断**整个发布流程，并输出标准错误（stderr）。CLI 会显示失败的具体命令及其 stdout/stderr 输出。
- **自动回滚**：中断后，CLI 会自动将 `package.json` 中的版本号回滚到修改前。
- **注意**：CLI 不会自动撤销钩子内已执行的其他操作（如已产生的 `git add`、已执行的脚本副作用），因此确保你的钩子具备幂等性（可重复安全执行）。

## FAQ

### Q: 如何跳过钩子？

A: 目前不支持运行时跳过钩子。如果需要临时禁用，只能注释掉 `pagoda.config.mjs` 中 `hooks.beforeRelease` 的配置项。

### Q: 钩子命令在哪个目录执行？

A: 所有钩子命令在项目根目录（即 `pagoda.config.mjs` 所在目录）执行，与你手动在终端中运行命令的环境一致。
