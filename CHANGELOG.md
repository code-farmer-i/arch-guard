...
> - **规模**：规则 98 → **107** · 夹具 78 → **93** · 需求 84 → 118（`已完成` 107 / `已委派` 4 / `不做` 7）。
>
> **⚠️ 发布前必做（尚未做）**：`engines: >= 22.18.0` 是对外承诺 —— 必须在 **Node 22.18** 上补跑一次
> `pnpm check`（见 `AGENTS.md` 的"提交前"一节；0.4.0 时跑过，361 测试全绿）。本节的数字目前**只在
> Node 24 上验证过**（406 测试 / 93 夹具 / 示例 102·91·71 满分 107）。跑完把结果补进本行。
