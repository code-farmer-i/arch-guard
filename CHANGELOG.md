...
- **已记录的技术债**：`finding()` 构造器在规则里有 **7 份**拷贝（`structure-util` / `design-shared` /
  `structure` / `copy` / `deps-adapters` / `hygiene-context` / `deps`）—— 收敛它要动 6 个文件的导入与定义，
  试过一次把相邻的辅助函数切掉了（当场回滚）。留作单独一批：收敛后再加字段就只有一处要改。
