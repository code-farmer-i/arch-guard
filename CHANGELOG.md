...
- 同一个文件里加**示例预算**：`examples/full` 必须 0 停用，`examples/minimal` 的停用条数不许超过 29
  （少配一个面 = 悄悄少一层覆盖）。
...
- 新宿主示例 [`examples/full/`](./examples/full/)：一个范式（`canonical`）+ 5 个域预设 + 8 个方案面 +
  结构声明全配的最小应用（62 个文件、4 个域）。**`pnpm guard:full` 跑出来：`规则 98/98` · 0 finding · 0 停用**
